...
function AlchemyAPI() {
	...
	// Load the key from the system variable
  var key = process.env.ALCHEMY_API_KEY;

  if (key === undefined) {
    console.log('ALCHEMYAPI_KEY environment variable is not set.')
    process.exit(1);
  }

	//Make sure the key formating looks good
	if (key.length != 40) {
		console.log('The API key does not appear to be valid. Make sure your environment variables are set correctly');
